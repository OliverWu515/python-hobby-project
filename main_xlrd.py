                ...
                try:
                    if re.match(re_lesson, item):
                        ...
                        if re.match(re_number, item).groups()[0][-1] == '周':  # (非实验课)
                            ...
                            ws_1.write(index, 2, re.match(re_lesson_experiment, st).groups()[0])  # 开始节数
                            ws_1.write(index, 3, re.match(re_lesson_experiment, st).groups()[1])  # 结束节数
        ...
        for col_index in range(1, ncols):
            for row_index in range(3, nrows):
                if table.cell_value(row_index, col_index) != "":
                    ...
                    lar = ar.split('\n')
                    ind = data_process(lar, ind, col_index)
        # 表头
        ws_1.write(0, 0, "课程名称")
        ws_1.write(0, 1, "星期")
        ws_1.write(0, 2, "开始节数")
